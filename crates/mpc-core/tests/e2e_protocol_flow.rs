...
use mpc_core::dkls23::{
    dkg::DkgSession,
    presign::{PresignSession, decode_presign_data},
...
/// Test 7: Full DKG → Presign → Sign pipeline (distributed, uses Paillier).
///
/// Simulates the production flow:
/// 1. DKG between device(0), server(1), backup(2)
/// 2. Presign between device(0) and server(1) — pre-compute nonces
/// 3. Sign a message using presign data for fast Round 1
/// 4. Verify signature
///
/// Note: Uses Paillier cryptography which is computationally expensive.
/// Run with: cargo test test_full_dkg_presign_sign -- --ignored
#[test]
#[ignore]
fn test_full_dkg_presign_sign() {
    // ========== PHASE 1: Distributed Key Generation ==========
    let session_id = "e2e-presign-pipeline";
    ...
    let device_share = dkg_device.process_round2(device_r2_msgs).unwrap();
    let server_share = dkg_server.process_round2(server_r2_msgs).unwrap();

    assert_eq!(device_share.public_key, server_share.public_key);
    println!("✓ DKG complete");

    // ========== PHASE 2: Distributed Presign ==========
    let mut ps_device = PresignSession::new(make_config("presign-001", 0));
    let mut ps_server = PresignSession::new(make_config("presign-001", 1));

    let ps_r1_device = ps_device.generate_round1().unwrap();
    let ps_r1_server = ps_server.generate_round1().unwrap();

    ps_device.process_round1(ps_r1_server).unwrap();
    ps_server.process_round1(ps_r1_device).unwrap();

    assert!(ps_device.is_complete());
    assert!(ps_server.is_complete());

    let presig_device = ps_device.finalize().unwrap();
    let presig_server = ps_server.finalize().unwrap();

    let data_device = decode_presign_data(&presig_device).unwrap();
    let data_server = decode_presign_data(&presig_server).unwrap();

    assert_eq!(data_device.aggregate_r, data_server.aggregate_r);
    assert_eq!(data_device.r_scalar, data_server.r_scalar);
    assert_ne!(data_device.k_i, data_server.k_i);
    println!("✓ Presign complete: aggregate R matches between parties");

    // ========== PHASE 3: Fast Sign with Presign Data ==========
    let message = b"Transfer 1 ETH using presign fast path";
    let msg_hash: [u8; 32] = sha3::Keccak256::digest(message).into();

    let mut sign_device = SignSession::new_distributed(
        make_config("sign-presign-001", 0),
        device_share.clone(),
        msg_hash,
    );
    let mut sign_server = SignSession::new_distributed(
        make_config("sign-presign-001", 1),
        server_share.clone(),
        msg_hash,
    );

    // Use pre-computed k and R for Round 1 (fast path — no fresh nonce generation)
    let sign_r1_device = sign_device
        .generate_round1_with_presign(&data_device.k_i, &data_device.r_i)
        .unwrap();
    let sign_r1_server = sign_server
        .generate_round1_with_presign(&data_server.k_i, &data_server.r_i)
        .unwrap();

    sign_device.process_round1(vec![sign_r1_server]).unwrap();
    sign_server.process_round1(vec![sign_r1_device]).unwrap();

    // Round 2: MtA + signature generation
    let sign_r2_device = sign_device.generate_round2().unwrap();
    let _sig_server = sign_server.process_round2(vec![sign_r2_device]).unwrap();

    let server_response = sign_server.get_server_response().unwrap();
    let server_response_msg = ProtocolMessage {
        session_id: "sign-presign-001".into(),
        from: 1,
        to: 0,
        round: 2,
        payload: server_response,
    ...
    // ========== PHASE 4: Verify ==========
    assert!(
        final_sig.verify(&msg_hash, &device_share.public_key).unwrap(),
        "presign pipeline: signature verification failed"
    );

    println!("✓ DKG → Presign → Sign pipeline complete: signature valid");
    println!("  Recovery ID: {}", final_sig.v);
}

/// Test 8: DKG → Presign pipeline (fast, no Paillier).
///
/// Validates:
/// 1. DKG produces consistent shares across 3 parties
/// 2. Presign between device(0) and server(1) produces matching aggregate R
/// 3. Presign data decodes correctly with expected fields
/// 4. Local sign with shares still works (integration sanity check)
#[test]
fn test_dkg_presign_pipeline_fast() {
    let session_id = "e2e-presign-fast";
    ...
    let device_r2: Vec<_> = vec![&r2_server, &r2_backup]
        .into_iter()
        .flat_map(|msgs| msgs.iter().filter(|m| m.to == 0))
        .cloned()
        .collect();
    let server_r2: Vec<_> = vec![&r2_device, &r2_backup]
        .into_iter()
        .flat_map(|msgs| msgs.iter().filter(|m| m.to == 1))
        .cloned()
        .collect();

    let device_share = dkg_device.process_round2(device_r2).unwrap();
    let server_share = dkg_server.process_round2(server_r2).unwrap();
    assert_eq!(device_share.public_key, server_share.public_key);

    // Presign
    let mut ps_device = PresignSession::new(make_config("ps-fast", 0));
    let mut ps_server = PresignSession::new(make_config("ps-fast", 1));

    let ps_r1_d = ps_device.generate_round1().unwrap();
    let ps_r1_s = ps_server.generate_round1().unwrap();
    ps_device.process_round1(ps_r1_s).unwrap();
    ps_server.process_round1(ps_r1_d).unwrap();

    assert!(ps_device.is_complete());
    assert!(ps_server.is_complete());

    let presig_d = ps_device.finalize().unwrap();
    let presig_s = ps_server.finalize().unwrap();

    let data_d = decode_presign_data(&presig_d).unwrap();
    let data_s = decode_presign_data(&presig_s).unwrap();

    // Verify presign properties
    assert_eq!(data_d.aggregate_r, data_s.aggregate_r);
    assert_eq!(data_d.r_scalar, data_s.r_scalar);
    assert_ne!(data_d.k_i, data_s.k_i);
    assert_eq!(data_d.r_i.len(), 33); // compressed point
    assert_eq!(data_d.aggregate_r.len(), 33);
    assert_eq!(data_d.k_i.len(), 32);
    assert_eq!(data_d.r_scalar.len(), 32);

    // Local sign as sanity check
    let msg_hash: [u8; 32] = sha3::Keccak256::digest(b"presign pipeline sanity").into();
    let mut sign = SignSession::new_local(
        make_config(session_id, 0),
        vec![0, 1],
        vec![device_share.clone(), server_share.clone()],
        msg_hash,
    );
    let sig = sign.sign_local().unwrap();
    assert!(sig.verify(&msg_hash, &device_share.public_key).unwrap());
}

/// Test 9: DKG then Reshare protocol flow (fast version with local signing).
///
/// Tests:
